...
void main() {
  group('Week2 UI画面実装検証', () {
    test('LoginScreen実装確認', () {
      ...
      expect(content.contains('ConsumerWidget'), isTrue,
          reason: 'ConsumerWidgetを継承していること');
      expect(content.contains('signInWithGoogle'), isTrue,
          reason: 'Google OAuth機能が実装されていること');
      expect(content.contains('Googleでログイン'), isTrue,
          reason: 'ログインボタンが存在すること');
    });
    ...
    test('ListScreen実装確認', () {
      ...
      final content = file.readAsStringSync();
      expect(content.contains('class ListScreen'), isTrue,
          reason: 'ListScreenクラスが定義されていること');
      expect(content.contains('ConsumerStatefulWidget'), isTrue,
          reason: 'ConsumerStatefulWidgetを継承していること');
      expect(content.contains('_loadItems'), isTrue,
          reason: 'アイテム読み込み機能が実装されていること');
      expect(content.contains('_toggleItem'), isTrue,
          reason: 'アイテムチェック機能が実装されていること');
      expect(content.contains('_addItem'), isTrue,
          reason: 'アイテム追加機能が実装されていること');
      expect(content.contains('ListView.builder'), isTrue,
          reason: 'リスト表示が実装されていること');
      expect(content.contains('FloatingActionButton'), isTrue,
          reason: 'FABが実装されていること');
      expect(content.contains('QRScreen'), isTrue,
    ...
    test('QRScreen実装確認', () {
      ...
      final content = file.readAsStringSync();
      expect(content.contains('class QRScreen'), isTrue,
          reason: 'QRScreenクラスが定義されていること');
      expect(content.contains('ConsumerWidget'), isTrue,
          reason: 'ConsumerWidgetを継承していること');
      expect(content.contains('familyId'), isTrue,
          reason: 'familyIdパラメータが定義されていること');
      expect(content.contains('Clipboard.setData'), isTrue,
          reason: '家族IDコピー機能が実装されていること');
      expect(content.contains('joinFamily'), isTrue,
          reason: '家族参加機能が実装されていること');
      expect(content.contains('家族IDをコピー'), isTrue,
          reason: 'コピーボタンが存在すること');
      expect(content.contains('QRコードスキャン'), isTrue,
          reason: 'スキャンボタンが存在すること');
    });
    ...
    test('SettingsScreen実装確認', () {
      ...
      expect(content.contains('signOut'), isTrue,
          reason: 'ログアウト機能が実装されていること');
      expect(content.contains('ログアウト'), isTrue,
          reason: 'ログアウトボタンが存在すること');
      expect(content.contains('popUntil'), isTrue,
    ...
    test('main.dart統合確認', () {
      ...
      expect(content.contains('home: const LoginScreen()'), isTrue,
    ...
    test('画面間ナビゲーション統合確認', () {
      final listScreen = File('lib/screens/list_screen.dart').readAsStringSync();

      expect(listScreen.contains('Navigator.push'), isTrue,
          reason: 'Navigator.pushが実装されていること');
      expect(listScreen.contains('MaterialPageRoute'), isTrue,
          reason: 'MaterialPageRouteが使用されていること');
      expect(
          listScreen.contains('QRScreen(familyId: widget.familyId)'), isTrue,
          reason: 'QR画面へのナビゲーションでfamilyIdが渡されていること');
      expect(listScreen.contains('SettingsScreen()'), isTrue,
          reason: '設定画面へのナビゲーションが実装されていること');
    });

    test('エラーハンドリング実装確認', () {
      final loginScreen = File('lib/screens/login_screen.dart').readAsStringSync();
      final listScreen = File('lib/screens/list_screen.dart').readAsStringSync();
      final qrScreen = File('lib/screens/qr_screen.dart').readAsStringSync();
      final settingsScreen = File('lib/screens/settings_screen.dart').readAsStringSync();

      // 全画面でtry-catchとSnackBarによるエラー表示が実装されていること
      expect(loginScreen.contains('try {') && loginScreen.contains('catch (e)'), isTrue,
          reason: 'LoginScreenにエラーハンドリングが実装されていること');
      expect(listScreen.contains('try {') && listScreen.contains('catch (e)'), isTrue,
          reason: 'ListScreenにエラーハンドリングが実装されていること');
      expect(qrScreen.contains('try {') && qrScreen.contains('catch (e)'), isTrue,
          reason: 'QRScreenにエラーハンドリングが実装されていること');
      expect(settingsScreen.contains('try {') && settingsScreen.contains('catch (e)'), isTrue,
          reason: 'SettingsScreenにエラーハンドリングが実装されていること');

      expect(loginScreen.contains('ScaffoldMessenger'), isTrue,
          reason: 'LoginScreenでSnackBarが使用されていること');
      expect(listScreen.contains('ScaffoldMessenger'), isTrue,
          reason: 'ListScreenでSnackBarが使用されていること');
      expect(qrScreen.contains('ScaffoldMessenger'), isTrue,
          reason: 'QRScreenでSnackBarが使用されていること');
      expect(settingsScreen.contains('ScaffoldMessenger'), isTrue,
    ...
    test('Week2実装完了確認', () {
      ...
      expect(loginScreenImplemented, isTrue,
          reason: 'LoginScreen(47行)実装完了');
      expect(listScreenImplemented, isTrue,
          reason: 'ListScreen(185行)実装完了');
      expect(qrScreenImplemented, isTrue,
          reason: 'QRScreen(115行)実装完了');
      expect(settingsScreenImplemented, isTrue,
          reason: 'SettingsScreen(61行)実装完了');
      expect(navigationIntegrated, isTrue,
          reason: '画面間ナビゲーション統合完了');
  ...
  group('MVP要件準拠確認', () {
    test('コード行数制限確認', () {
      final loginScreen = File('lib/screens/login_screen.dart')
          .readAsLinesSync()
          .where((line) => line.trim().isNotEmpty)
          .length;
      final listScreen = File('lib/screens/list_screen.dart')
          .readAsLinesSync()
          .where((line) => line.trim().isNotEmpty)
          .length;
      final qrScreen = File('lib/screens/qr_screen.dart')
          .readAsLinesSync()
          .where((line) => line.trim().isNotEmpty)
          .length;
      final settingsScreen = File('lib/screens/settings_screen.dart')
          .readAsLinesSync()
          .where((line) => line.trim().isNotEmpty)
          .length;
    ...
    test('カスタムコンポーネント不使用確認', () {
      ...
      expect(customWidgetsDir.existsSync(), isFalse,
      ...
      for (final screenPath in screens) {
        ...
        expect(classCount, lessThanOrEqualTo(2),
