...
**重要**: 親・子の役割は**データベースに保存しない**。ローカルデバイス上での**UIフロー分岐のためだけ**に使用する。

| 役割 | 定義 | UI上の違い | データベース保存 |
|------|------|------------|------------------|
| **親** | QRコード生成権限を持つユーザー | 初回登録時に家族グループを自動作成し、QRコード表示画面に遷移 | ❌ 保存しない |
| **子** | QRコードスキャンでグループ参加するユーザー | 初回登録時にQRスキャン画面に遷移し、家族参加必須 | ❌ 保存しない |
...
**保存しないデータ**:
- ❌ **役割（親/子）**: UIフロー分岐のみに使用、データベース同期不要
...
| キー | 保存内容 | 用途 |
|-----|---------|------|
| `user_role` | "parent" or "child" | 初回登録フロー分岐 |
...
- 🎯 **MVP簡素化**: データベーススキーマを最小限に保つ

#### 5.4.3 データ整合性の考慮事項

**ローカルデータの信頼性**:
- ⚠️ ローカルの役割データは参照情報のみ（認可には使用しない）
- ⚠️ アプリ再インストール時はローカルデータ消失（初回登録フローを再実行）
- ⚠️ 複数デバイスで異なる役割を選択可能（仕様として許容）

**セキュリティへの影響**:
- ✅ 権限制御をローカルデータに依存しないため、改ざんリスクなし
...
| 項目 | 制約内容 | 影響 |
|------|---------|------|
| **役割の変更** | アプリ内で役割変更機能を提供しない | 再インストール時に変更可能 |
| **役割の検証** | サーバー側で役割の検証を行わない | クライアント側でUIフロー制御のみ |
| **複数デバイス** | 同一ユーザーが異なるデバイスで異なる役割を選択可能 | 仕様として許容（MVP） |
...
**将来のv2.0での拡張想定**:
1. ✅ `users`テーブルに`role`カラム追加（マイグレーション容易）
2. ✅ RLSポリシー追加で権限管理実装（既存データ影響なし）
3. ✅ 複数家族グループ対応（スキーマ変更最小限）

**後方互換性**:
- ✅ MVP版ユーザーがv2.0にアップデートした際、役割を再選択するフロー
- ✅ 既存データはすべて引き継ぎ可能
